          ...
          cartData.map((item,index) => {
            ...
            return (
              <div key={index} className='py-4 border-t border-b border border-black text-gray-700 grid grid-cols-[4fr_0.5fr_0.5fr] sm:grid-cols-[4fr_2fr_0.5fr] items-center gap-4 bg-pink-100'>
                ...
                <input onChange={(e)=> e.target.value === '' || e.target.value === '0' ? null : updateQuantity(item._id,item.size,Number(e.target.value))} className=' border border-black max-w-10 sm:max-w-20 px-1 sm:px-2 py-1 bg-violet-100' type="number" min={1} defaultValue={item.quantity} />
