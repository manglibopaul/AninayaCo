...
if (!nameArg) {
  console.error('Usage: node deleteProduct.js "Product Name"');
  process.exit(1);
}
...
const run = async () => {
  try {
    ...
    // Confirm deletion with user via console prompt
    const confirm = await new Promise(resolve => {
      process.stdout.write('Delete these products? Type YES to confirm: ');
      process.stdin.setEncoding('utf8');
      process.stdin.once('data', (d) => resolve(String(d).trim()));
    });

    if (confirm !== 'YES') {
      console.log('Aborted. No products were deleted.');
      process.exit(0);
    }
